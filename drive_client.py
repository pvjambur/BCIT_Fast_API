...
class DriveClient:
    ...
    def process_drive(self):
        ...
        try:
            ...
            # Group folders by their base name
            folder_groups = {}
            for folder in folders:
                base_name = folder['title']
                # Remove any existing suffix (_1, _2 etc.)
            ...
            for base_name, folders in folder_groups.items():
                if len(folders) == 1:
                    # Single folder, process normally
                    self.process_folder(folders[0])
                else:
                    # Multiple folders with same name, add suffix
                    for i, folder in enumerate(folders, 1):
                        self.process_folder(folder, suffix=f"_{i}")
                
        except Exception as e:
            logger.error(f"Error processing drive: {str(e)}")

    def process_folder(self, folder, suffix=""):
        folder_name = folder['title']
        # Apply suffix if provided
        safe_folder_name = f"{folder_name.replace(' ', '_')}{suffix}"
        local_folder = Path(self.local_temp) / safe_folder_name
        local_folder.mkdir(exist_ok=True)
        
        logger.info(f"Processing folder: {safe_folder_name}")
